...
class Apiki_Buscape_API {
	...
	protected function _getContent( $serviceName , array $args , $lomadee = false ) {
		...
		$url = sprintf( 'http://%s.buscape.com/service/%s/%s/%s/?%s' , $this->_environment , $serviceName , $this->_applicationId , $this->_countryCode , http_build_query( $args ) );
                //$url = sprintf( 'http://sandbox.buscape.com/service/%s/%s/%s/?%s', $serviceName , $this->_applicationId , $this->_countryCode , http_build_query( $args ) );
                                //http://sandbox.buscape.com/service/findProductList/7838643852314c587376343d/br/?categoryId=5817&sourceId=9262544
		$curl = curl_init();
